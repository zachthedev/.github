...
When a process is still running at its deadline, the gate also starts the system's own `taskkill` on Windows,
from the system directory, or `/bin/ps` or `/usr/bin/ps` elsewhere, never through `PATH`, to end it and every
process it started. A process that already exited is never killed by its pid.
