...
- The gate refuses the forms golangci-lint honors and nolintlint misses:
  ...
  - a generated marker golangci-lint's `lax` mode reads, in any tracked `.go` file the generator does not list:
    ...
    above `package` or in the comment group right after it.
...
- StyleCop's SA1404 requires a `Justification` on every `[SuppressMessage]` and rejects an empty one or
  `<Pending>`. Every other StyleCop rule is off. StyleCop's newest release is a prerelease from 2023, which the
  owner accepted, pinned exactly.
...
- The gate also refuses:
  ...
  - Roslyn's generated markers: a file name Roslyn reads as generated, a leading `<auto-generated` or
    `<autogenerated` comment, and `[GeneratedCode`. A generated file loses its analyzers and its nullable checks;
  - `[UnconditionalSuppressMessage]`, which SA1404 does not read;
  - CSharpier's ignore comments.
