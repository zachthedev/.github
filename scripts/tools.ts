...
const PinsSchema = z.object({
  ...
  settings: z.object({
    lockfile_platforms: z.array(z.string()).min(1),
  }),
});
...
const LockSchema = z.object({
  tools: z.record(z.string(), z.array(EntrySchema)),
});
...
export async function lockfileFindings(): Promise<string[]> {
  const pins = PinsSchema.safeParse(Bun.TOML.parse(await Bun.file(PINS).text()));
  if (!pins.success) {
    return [`${PINS} must carry [tools] and [settings] lockfile_platforms: ${pins.error.message}`];
  }
  const lock = LockSchema.safeParse(Bun.TOML.parse(await Bun.file(LOCK).text()));
  if (!lock.success) {
    return [`${LOCK} is not a lockfile: ${lock.error.message}`];
  }

  const found: string[] = [];
  const pinned = Object.keys(pins.data.tools).sort();
  ...
  for (const tool of TOOLS) {
    const version = pins.data.tools[tool.key];
    ...
    if (entry.version !== version) {
      found.push(
        `${PINS} pins ${tool.key} ${version}, and ${LOCK} records ${entry.version}. Write it again with: ${RELOCK}`,
      );
    }
    const coordinate = `aqua:${tool.owner}/${tool.repository}`;
    if (entry.backend !== coordinate) {
      found.push(`${LOCK} records backend ${entry.backend} for ${tool.key}, and the tool comes from ${coordinate}`);
    }
    ...
    for (const platform of pins.data.settings.lockfile_platforms) {
    ...
    for (const platform of recorded) {
      if (!pins.data.settings.lockfile_platforms.includes(platform)) {
        found.push(
...
export async function resolve(): Promise<ReadonlyMap<string, string>> {
  const pins = PinsSchema.parse(Bun.TOML.parse(await Bun.file(PINS).text()));
  const resolved = new Map<string, string>();
  for (const tool of TOOLS) {
    const version = pins.tools[tool.key];
    if (version === undefined) {
